...
namespace Epam.TaskFinal.Blog.App_Code.Data
{
    public class PostRepository
    {
        ...
        public static IEnumerable<dynamic> GetPublishedPosts(int count = 0)
        {
            ...
            var sql = string.Format("SELECT {0} p.*, t.Id as TagId, t.Name as TagName, t.UrlFriendlyName " +
        ...
        public static dynamic Get(int id)
        {
            var sql = "SELECT p.*, t.Id as TagId, t.Name as TagName, t.UrlFriendlyName " +
                      ...
                      "LEFT JOIN PostsTagsMap m ON p.Id = m.PostId " +
                      "LEFT JOIN Tag t ON t.Id = m.TagId " +
                      "INNER JOIN Users u ON u.Id = p.AuthorId WHERE p.Id = @0";
        ...
        public static dynamic Get(string slug)
        {
            ...
            "LEFT JOIN PostsTagsMap m ON p.Id = m.PostId " +
            "LEFT JOIN Tag t ON t.Id = m.TagId " +
            "INNER JOIN Users u ON u.Id = p.AuthorId WHERE Slug = @0";
        ...
        public static void Add(string title, string content, string slug, DateTime? datePublished, int authorId, IEnumerable<int> tags)
        {
            ...
            {
                var sql = "INSERT INTO Post (Title, Content, DatePublished, AuthorId, Slug) " +
                    "VALUES (@0, @1, @2, @3, @4)";
                db.Execute(sql, title, content, datePublished, authorId, slug);

                var post = db.QuerySingle("SELECT * FROM Post WHERE Slug = @0", slug);
        ...
        public static void Edit(int id, string title, string content, string slug, DateTime? datePublished, int authorId, IEnumerable<int> tags)
        {
            using (var db = Database.Open(_connectionString))
            {
                var sql = "UPDATE Post SET Title = @0, Content = @1, DatePublished = @2, AuthorId = @3, Slug = @4 " +
                    "WHERE Id = @5";
        ...
        public static void Remove(string slug)
        {
            using (var db = Database.Open(_connectionString))
            {
                var sql = "SELECT * FROM Post WHERE Slug = @0";
        ...
        private static dynamic CreatePostOject(dynamic obj)
        {
